...
机器人运行时会自动采集统计并写入 `data/db/stats.db`（可在面板"数据库管理"中查看/编辑），网页管理面板主页（精简总次数）与"数据统计"页（分类分区 + 年/月/日/小时粒度折线图/文本详情）实时展示：

| 指标 | 口径 |
|---|---|
| 信息接收/发送次数 | 收到的 QQ 消息事件（群聊/私聊，post_type=message）/ 调用发送消息 API 的次数 |
| 信息接收发送总次数 | 前两者之和（面板计算） |
| 接口上行/下行次数 | 从 OneBot 收到的全部事件（不含心跳）/ 调用 OneBot API 的总次数（含统计自身的 get_group_list 轮询） |
| 接口上下行总次数 | 前两者之和（面板计算） |
| 信息/指令频率（条/day） | **历史日均** = 总量 ÷ 统计天数（自首次记录起，不足 1 天按 1 天计） |
| 指令触发次数 | 被核心内置功能或插件成功处理的消息事件数 |
| 最常触发指令（不计插件） | 全部插件 + 内置指令**统一**按指令名排行 |
| 最常触发指令（按插件分类） | 按 plugin_id 归类（核心内置指令归"核心内置"组），显示各分类触发总数与其最常触发指令 |
| 已添加场景数 | 群聊（get_group_list 每 5 分钟轮询 + 消息事件补充）/ 私聊（自统计上线累计） |
| 已启用插件场景数 | permissions.db 中启用 ≥1 个插件的场景（群聊/私聊） |

存储（`data/db/stats.db`）：`counters`（总次数）、`buckets`（小时桶预聚合，时序/频率源）、`cmd_events`（指令明细，排行源）、`scenes`（场景）。计数在 `src/core/stats.py` 中采集（`main.py` 入口钩子 + `handle_message.py` 指令钩子），时序/明细保留约 2 年；重启不清零（持久化于 SQLite）。

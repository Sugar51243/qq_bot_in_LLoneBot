...
- **数据统计** `/stats.html`（主页亦有精简区块）：展示机器人实时采集的 7 项计数（信息接收/发送/总计、接口上行/下行/总计、指令触发，数据存 `data/db/stats.db`，`GET /api/stats` 只读拉取）；每 10 秒自动刷新，页面隐藏时暂停；机器人未运行或库不存在时显示"暂无数据"（不报错）。

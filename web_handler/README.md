...
- **多语言**：顶栏（登录页为右上角）的 🌐 下拉可切换 **简体中文 / 繁體中文 / English**，即时生效不刷新页面；选择存于浏览器 localStorage，首次访问按浏览器语言自动选择。服务端返回的错误提示（如登录失败原因）仍为服务端文案。
- **多主题**：顶栏 🎨 面板提供 **青绿 / 浅蓝（默认）/ 白色 / 黑色** 四套配色，另支持 **自定义背景**：可选背景颜色（颜色选择器）或背景图片（受 CSP 限制，仅支持同源路径如 `/data/xxx.png` 或 `data:image/...` URL；图片与颜色同时设置时以图片为准）。
- **移动端**：窄屏下顶栏导航折叠为 ☰ 菜单；文件行单列堆叠；编辑器/日志/预览/弹窗全屏化；数据库页左侧列表变为可折叠抽屉（页首 🗄 按钮开关，选择表格后自动收起）；表格保持横向滚动。主题/语言选择同样在移动端可用。
- 语言与主题选择均在**浏览器本地**保存（`xsw_lang` / `xsw_theme` / `xsw_custom_bg`），换设备/换浏览器需重新设置，面板不感知这些偏好。
